...
import npp.builder.RandomPanelBuilder;
import npp.command.CommandHistory;
import npp.command.PressButtonCommand;
import npp.command.ReleaseButtonCommand;
import npp.composite.PanelGroup;
import npp.container.Cell;
 ...
 *
 * Spring Bean — синглтон. Является основным управляющим компонентом приложения.
 * CommandHistory и ObjectProvider<RandomPanelBuilder> внедряются через DI.
 *
 * ObjectProvider используется для получения прототипных бинов RandomPanelBuilder —
 * каждый раз создаётся новый экземпляр строителя для новой панели.
 */
@Component
public class MenuController {
    ...
    private final ObjectProvider<RandomPanelBuilder> builderProvider;
    ...
    public MenuController(final CommandHistory commandHistory,
                          final ObjectProvider<RandomPanelBuilder> builderProvider) {
        this.commandHistory = commandHistory;
        this.builderProvider = builderProvider;
        // Явно указываем UTF-8, чтобы русский ввод читался корректно
    ...
    private void printWelcome() {
        ...
        System.out.println("|   Controlpanel_of_NPP (Spring)       |");
        System.out.println("+--------------------------------------+");
    }
    ...
    private boolean handleMainChoice(final int choice) {
        switch (choice) {
            case 1: System.out.println(panel.render()); break;
            case 2: generatePanelWithParams(); break;
            case 3: pressButtonMenu(); break;
            case 4: releaseButtonMenu(); break;
            case 5:
                commandHistory.undoLast();
                System.out.println(panel.render());
                break;
            case 6: showStatus(); break;
            case 7: showCompositeStructure(); break;
            case 8: showBindings(); break;
            case 0: return false;
    ...
    /**
     * Получаем новый прототипный бин RandomPanelBuilder через ObjectProvider,
     * чтобы каждая панель строилась независимым строителем.
     */
    private void buildPanel(final int width, final int height, final long seed) {
        RandomPanelBuilder builder = builderProvider.getObject();
    ...
    private void pressButtonMenu() {
        if (buttons.isEmpty()) { System.out.println("На панели нет кнопок."); return; }
        System.out.println("\n--- Нажатие кнопки ---");
        printButtonList();
        int index = readIntInRange("Номер кнопки (1-" + buttons.size() + "): ", 1, buttons.size());
        Button button = buttons.get(index - 1);
        if (button.isPressed()) {
            System.out.println("Кнопка '" + button.getName() + "' уже нажата."); return;
        }
    ...
    private void releaseButtonMenu() {
        ...
        Button button = buttons.get(index - 1);
        if (!button.isPressed()) {
            System.out.println("Кнопка '" + button.getName() + "' уже отпущена."); return;
        }
